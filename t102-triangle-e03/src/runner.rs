...
pub async fn run(event_loop: EventLoop<()>, window: Window) {
    ...
    let render_pipline_yellow = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("Render Pipline Yellow"),
        layout: Some(&render_pipline_layout),
        vertex: wgpu::VertexState {
            module: &shader,
            entry_point: "vs_main",
    ...
    let (vertices1, indices1) = Vertex::triangle1();
    ...
    let (vertices2, indices2) = Vertex::triangle2();
    ...
    event_loop.run(move |event, _, control_flow| match event {
        ...
        winit::event::Event::RedrawRequested(window_id) if window_id == window.id() => {
                ...
                device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
            {
                ...
                render_pass.set_index_buffer(index_buffer1.slice(..), wgpu::IndexFormat::Uint32);
                render_pass.draw_indexed(0..indices1.len() as u32, 0, 0..1);

                render_pass.set_pipeline(&render_pipline_yellow);
                render_pass.set_vertex_buffer(0, vertex_buffer2.slice(..));
                render_pass.set_index_buffer(index_buffer2.slice(..), wgpu::IndexFormat::Uint32);
                render_pass.draw_indexed(0..indices2.len() as u32, 0, 0..1);
            }
