...
pub async fn run(event_loop: EventLoop<()>, window: Window) -> Result<()> {
    ...
    let mut light_direction_arr = vec![
        LightDirection::new(
            [0.0, 0.0, -1.0],
            [1.0, 1.0, 1.0],
            [0.01, 0.01, 0.01],
    ...
    let mut light_spot_arr = vec![LightSpot::new(
        camera.pos.into(),
        camera.front.into(),
        [0.0, 0.0, 1.0],
        [0.1, 0.1, 0.1],
    ...
    let light_point_transform_arr = light_point_arr
        .iter()
        .map(|p| Transform::new(p.pos.into(), Quat::IDENTITY, Vec3::splat(0.2)))
        .collect::<Vec<_>>();
    let light_point_transform_raw_arr = light_point_transform_arr
        .iter()
        .map(|t| t.to_raw())
        .collect::<Vec<_>>();
    let light_point_transform_buffer =
        device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Light Transform Buffer"),
            contents: bytemuck::cast_slice(&light_point_transform_raw_arr),
            usage: wgpu::BufferUsages::VERTEX,
        });

    let light_point_color_arr = light_point_arr.iter().map(|p| p.color).collect::<Vec<_>>();
    let light_point_color_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Light Point Color Buffer"),
        contents: bytemuck::cast_slice(&light_point_color_arr),
        usage: wgpu::BufferUsages::VERTEX,
    });
    ...
    event_loop.run(move |event, _, control_flow| match event {
        winit::event::Event::RedrawRequested(window_id) if window_id == window.id() => {
            ...
            light_spot_arr[0].pos = camera.pos.into();
            light_spot_arr[0].front = camera.front.into();
            queue.write_buffer(&light_spot_buffer, 0, bytemuck::cast_slice(&light_spot_arr));

            {
                ...
                render_pass.set_vertex_buffer(0, vertex_buffer.slice(..));
                render_pass.set_vertex_buffer(1, light_point_transform_buffer.slice(..));
                render_pass.set_vertex_buffer(2, light_point_color_buffer.slice(..));
                render_pass.set_bind_group(0, &view_proj_bind_group, &[]);
                render_pass.draw(0..vertices.len() as _, 0..light_point_arr.len() as u32);
            }
