...
pub async fn run(event_loop: EventLoop<()>, window: Window) {
    ...
    let mut transform0 = Transform::new(Vec3::new(0.5, -0.5, 0.0), Quat::IDENTITY, Vec3::ONE);
    let mut transform1 = Transform::new(Vec3::new(-0.5, 0.5, 0.0), Quat::IDENTITY, Vec3::ONE);
    ...
    event_loop.run(move |event, _, control_flow| match event {
        winit::event::Event::RedrawRequested(window_id) if window_id == window.id() => {
            ...
            let time_past = (Instant::now() - start_time).as_secs_f32();
            transform0.rotation = Quat::from_rotation_z(time_past);
            transform1.scale = Vec3::new(time_past.sin(), time_past.sin(), 1.0);

            let transform_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("Transform Buf"),
                contents: bytemuck::cast_slice(&[
                    transform0.to_mat4().to_cols_array_2d(),
                    transform1.to_mat4().to_cols_array_2d(),
                ]),
                usage: wgpu::BufferUsages::VERTEX,
            });
                ...
                device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
            {
                ...
                render_pass.set_vertex_buffer(1, transform_buf.slice(..));
