...
@group(3)@binding(3)
var texture_emission: texture_2d<f32>;
@group(3)@binding(4)
...
fn fs_main(in: VertexOut) -> @location(0) vec4<f32> {
    ...
    let tex_emission = textureSample(texture_emission, texture_sampler, in.tex_coord).rgb;
    ...
    let is_zero = tex_specular.r == 0.0 && tex_specular.g == 0.0 && tex_specular.b == 0.0;
    if is_zero {
        return vec4<f32>(ambient + diffuse + specular + tex_emission, 1.0);
    } else {
