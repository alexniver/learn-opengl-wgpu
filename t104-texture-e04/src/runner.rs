...
pub async fn run(event_loop: EventLoop<()>, window: Window) {
    ...
    let render_pipline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        ...
        bind_group_layouts: &[&texture_bind_group_layout, &bias_bind_group_layout],
    ...
    event_loop.run(move |event, _, control_flow| match event {
        winit::event::Event::RedrawRequested(window_id) if window_id == window.id() => {
            ...
            let bias_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
...

            {
                ...
                render_pass.set_bind_group(1, &bias_bind_group, &[]);
                    ...
                    if input.state == winit::event::ElementState::Released =>
                {
                    match input.virtual_keycode {
                        Some(winit::event::VirtualKeyCode::Up) => {
                            bias = (bias - delta_time * 10.0).max(0.0);
                        }
                        Some(winit::event::VirtualKeyCode::Down) => {
                            bias = (bias + delta_time * 10.0).min(1.0);
                        }
                        _ => {}
                    }
                }
