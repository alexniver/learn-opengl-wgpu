...
use crate::{
    texture::{gen_sampler, gen_texture_view},
    transform::Transform,
    vertex::Vertex,
};
...
pub async fn run(event_loop: EventLoop<()>, window: Window) {
    ...
    let texture_container_view = gen_texture_view("assets/texture/container.jpg", &device, &queue);
    let texture_huaji_view = gen_texture_view("assets/texture/huaji.jpg", &device, &queue);
    ...
    let texture_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            ...
            wgpu::BindGroupEntry {
                binding: 1,
                resource: wgpu::BindingResource::TextureView(&texture_container_view),
            },
            wgpu::BindGroupEntry {
                binding: 2,
                resource: wgpu::BindingResource::TextureView(&texture_huaji_view),
            },
        ],
    });

    let mut transform = Transform::new(Vec3::new(0.5, -0.5, 0.0), Quat::IDENTITY, Vec3::ONE);
    ...
    event_loop.run(move |event, _, control_flow| match event {
        winit::event::Event::RedrawRequested(window_id) if window_id == window.id() => {
            ...
            transform.rotation = Quat::from_rotation_z((Instant::now() - start_time).as_secs_f32());
            let transform_vertex_buffer =
                device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                    label: Some("Transform Vertex Buffer"),
                    contents: bytemuck::cast_slice(&transform.mat4().to_cols_array_2d()),
                    usage: wgpu::BufferUsages::VERTEX,
                });

            {
                ...
                render_pass.set_vertex_buffer(1, transform_vertex_buffer.slice(..));
