...
pub fn run() {
    ...
    pollster::block_on(async {
        ...
        for i in 0..amount {
            // pos
            let angel = i as f32 / amount as f32 * 360.0;
            let displacement = (rand() % (2.0 * offset * 100.0) as i32) as f32 / 100.0 - offset;
            let x = angel.sin() * radius + displacement;
            let y = displacement * 0.4;
            let displacement = (rand() % (2.0 * offset * 100.0) as i32) as f32 / 100.0 - offset;
            let z = angel.cos() * radius + displacement;

            let pos = Vec3::new(x, y, z);

            // rotation
            let rotation = Quat::from_axis_angle(Vec3::new(0.4, 0.6, 0.8), (rand() % 360) as f32);

            // scale
            let scale = Vec3::splat((rand() % 20) as f32 / 100.0 + 0.05);

            let transform = Transform::new(pos, rotation, scale);
            transform_arr.push(transform);
        }
        load_gltf_model("rock.gltf", &transform_arr, &mut core);
...
fn rand() -> i32 {
    ...
    (rng.gen::<f32>() * 2000.0) as i32
}

pub fn load_gltf_model(path: &str, transform_arr: &Vec<Transform>, core: &mut Core) {
    let base_path = std::path::Path::new(BASE_GLTF_PATH);
    println!("path: {:?}", path);

    let gltf_path = base_path.join(path);
