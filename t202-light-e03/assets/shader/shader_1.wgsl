...
struct VertexOut {
    ...
    @location(1) normal: vec3<f32>,
    @location(2) light_view_pos: vec3<f32>,
}
...
fn vs_main(in: VertexIn, transform: Transform) -> VertexOut {
    ...
    out.frag_pos = (view * model * vec4<f32>(in.pos, 1.0)).xyz;
    out.normal = in.normal;
    out.light_view_pos = (view * vec4<f32>(light_pos, 1.0)).xyz;
    return out;
}
...
fn fs_main(in: VertexOut) -> @location(0) vec4<f32> {
    ...
    let light_dir = normalize(in.light_view_pos - in.frag_pos);
