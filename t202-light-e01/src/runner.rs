...
pub async fn run(event_loop: EventLoop<()>, window: Window) -> Result<()> {
    ...
    let render_pipline_layout_1 = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        ...
        bind_group_layouts: &[&view_proj_bind_group_layout, &light_bind_group_layout],
    ...
    let light_color = [1.0_f32, 1.0_f32, 1.0_f32];
    let model_color = [1.0_f32, 0.5_f32, 0.31_f32];
    ...
    let model_color_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Model Color Buffer"),
        contents: bytemuck::cast_slice(&model_color),
        usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
    });

    let light_pos_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Light Pos Buffer"),
        contents: bytemuck::cast_slice(&light_pos.to_array()),
        usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
    });
    ...
    let light_color_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            wgpu::BindGroupEntry {
                ...
                resource: wgpu::BindingResource::Buffer(
                    light_color_buffer.as_entire_buffer_binding(),
                ),
            },
            wgpu::BindGroupEntry {
                binding: 1,
                resource: wgpu::BindingResource::Buffer(
                    model_color_buffer.as_entire_buffer_binding(),
                ),
            },
            wgpu::BindGroupEntry {
                binding: 2,
                resource: wgpu::BindingResource::Buffer(
                    light_pos_buffer.as_entire_buffer_binding(),
                ),
            },
            wgpu::BindGroupEntry {
                binding: 3,
    ...
    let mut light_transform = Transform::new(light_pos, Quat::IDENTITY, Vec3::splat(0.2));
    ...
    event_loop.run(move |event, _, control_flow| match event {
        winit::event::Event::RedrawRequested(window_id) if window_id == window.id() => {
            ...
            light_pos.z = total_time.sin() * 2.0;
            light_pos.x = total_time.cos() * 2.0;
            queue.write_buffer(
                &light_pos_buffer,
                0,
                bytemuck::cast_slice(&light_pos.to_array()),
            );
            light_transform = Transform::new(light_pos, Quat::IDENTITY, Vec3::splat(0.2));
            queue.write_buffer(
                &light_transform_buffer,
                0,
                bytemuck::cast_slice(&light_transform.to_mat4().to_cols_array_2d()),
            );

            {
                ...
                render_pass.set_bind_group(1, &light_color_bind_group, &[]);
