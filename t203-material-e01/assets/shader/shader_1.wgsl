...
fn fs_main(in: VertexOut) -> @location(0) vec4<f32> {
    let ambient = light.color * light.ambient * material.ambient;

    let normal = normalize(in.normal);
    let light_dir = normalize(light.pos - in.frag_pos);
    let diff = max(dot(normal, light_dir), 0.0);
    let diffuse = light.color * light.diffuse * (diff * material.diffuse);

    let view_dir = normalize(camera_pos - in.frag_pos);
    let reflect_dir = reflect(-light_dir, normal);
    let spec = pow(max(dot(view_dir, reflect_dir), 0.0), material.shininess);
    let specular = light.color * light.specular * (spec * material.specular);

    //return vec4<f32>((ambient + diffuse + specular) * (light.color), 1.0);
    return vec4<f32>((ambient + diffuse + specular) * (light.color * material.color), 1.0);
    //return vec4<f32>(material.shininess);
}
