...
pub async fn run(event_loop: EventLoop<()>, window: Window) -> Result<()> {
    ...
    let light_color_bind_group_layout =
        device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Light Color Bind Group Layout"),
            entries: &[wgpu::BindGroupLayoutEntry {
                binding: 0,
                visibility: wgpu::ShaderStages::FRAGMENT,
                ty: wgpu::BindingType::Buffer {
    ...
    let render_pipline_layout_2 = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("Render Pipline Layout"),
        bind_group_layouts: &[&view_proj_bind_group_layout, &light_color_bind_group_layout],
        push_constant_ranges: &[],
    });
    ...
    let light_color_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Light Color Buffer"),
        contents: bytemuck::cast_slice(&light.color),
        usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
    });
    let light_color_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("Light Color Bind Group"),
        layout: &light_color_bind_group_layout,
        entries: &[wgpu::BindGroupEntry {
            binding: 0,
            resource: wgpu::BindingResource::Buffer(light_color_buffer.as_entire_buffer_binding()),
        }],
    });
    ...
    event_loop.run(move |event, _, control_flow| match event {
        winit::event::Event::RedrawRequested(window_id) if window_id == window.id() => {
            ...
            queue.write_buffer(&light_color_buffer, 0, bytemuck::cast_slice(&light.color));
