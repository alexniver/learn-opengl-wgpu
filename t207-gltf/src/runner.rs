...
use crate::{
    ...
    texture::{gen_texture_sampler, gen_texture_view},
    transform::Transform,
    vertex::Vertex,
};
...
pub fn load_gltf_model(core: &mut Core) {
    ...
    for material in gltf_info.materials() {
        let texture_sampler = gen_texture_sampler(&core.device);
        ...
        let material = Material::new(texture_sampler, texture_view, 32.0, core);
...
fn box_material(core: &Core) -> Material {
    ...
    Material::new(texture_sampler, texture_diffuse_view, 32.0, core)
}
